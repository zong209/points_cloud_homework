...
def ground_segmentation(data):
    # 使用RANSAC进行平面拟合
    p = 0.99 #at least 能采样到once好数据的概率
    e = 0.7  #离群点的占比
    s = 3    #至少3个点
    ...
    dist_threshold = 0.1 #距离阈值
    best_a,best_b,best_c,best_d = 0,0,0,0
    print("Total iter:", N)
    while iter_num<N:
        if(iter_num%100==0):
            print("--cur iter:", iter_num)
        #随机三个采样点
    ...
    ground_cloud = data[inliers_points_bool==True]
    print('origin data points num:', data.shape[0])
    print('segmented data points num:', segmengted_cloud.shape[0])
    return segmengted_cloud,ground_cloud
...
def clustering(data):
    ...
    kmeans = cluster.k_means(data, 5)
    clusters_index = kmeans[1]
    # clusters_index =cluster.spectral_clustering(np.dot(data,data.T))
    # clusters_index = cluster.DBSCAN(min_samples=10).fit(data).labels_
...
def plot_clusters(data, ground, cluster_index):
    ...
    ax.scatter(ground[:, 0], ground[:, 1], ground[:, 2], s=2, color=colors[-1])
    plt.show()
...
def main():
    ...
    for i in range(iteration_num):
        ...
        plot_clusters(segmented_points,ground_points, cluster_index)
